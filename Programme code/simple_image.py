...
class Image:
    ...
    def _check_coordinate(self, xy):
        if (xy[0] < 0 or xy[0] >= self.width
            ...
            raise IndexError(f"Coordinate ({xy[0]},{xy[1]}) out of image {self.width}x{self.height}")

    def _check_color(self, color):
        for comp in color:
            if not isinstance(comp, int):
                raise TypeError(f"Color components ({color[0]},{color[1]},{color[2]}) should be integer values")
